...
export class ChatComponent implements OnInit {  
    ...
    constructor( private lf: FormBuilder, private chatService : ChatService , private socketService: SocketService, private customerService : CustomerService , private router: Router) {}
        ...
        if(exist != -1){                
            ...
            this.customerService.getOne(response.fromCustId).subscribe(itemr => {
                ...
                this.customerService.getmessage(newobj).subscribe(responsee => {
                    var obj = {fromCustId: this.cid, toCustId : response.fromCustId, email: itemr.message.email, firstname: itemr.message.firstname, lastname: itemr.message.lastname, fromSocketId :this.mysocketId, toSocketId : response.fromSocketId, openBox : 'open', messages : []};
                    var tobj = [];
                    if(localStorage.getItem('ChatList')){
    ...
    createNewChatBox(index, response){
        if(index == -1) {
            var obj3 : any = {fromCustId: this.cid, toCustId : response.customerdetail._id, email: response.customerdetail.email, firstname: response.customerdetail.firstname, lastname: response.customerdetail.lastname, fromSocketId : this.mysocketId, toSocketId : response.customerdetail.socketId, openBox : 'open', messages : [], typing : ""};
            var nr : any = [];            
            if(localStorage.getItem('ChatList')){
        ...
        if(this.oldchats.length == 0){
            for(var i=0; i<this.currentusers.chatList.length; i++){
                if(this.currentusers.chatList[i]._id != this.cid){
                    var chatboxi = {fromCustId: this.cid, toCustId : this.currentusers.chatList[i]._id, email: this.currentusers.chatList[i].email, firstname: this.currentusers.chatList[i].firstname, lastname: this.currentusers.chatList[i].lastname, fromSocketId :this.mysocketId, toSocketId : this.currentusers.chatList[i].socketId, openBox : 'close', messages : []};
            ...
            for(var i=0; i<this.currentusers.chatList.length; i++){
                ...
                if(newindex != -1){
                    if(this.currentusers.chatList[i]._id != this.cid){
                        ...
                        var inputa = {fromCustId: this.cid, toCustId : this.currentusers.chatList[i]._id, email: this.currentusers.chatList[i].email, firstname: this.currentusers.chatList[i].firstname, lastname: this.currentusers.chatList[i].lastname, fromSocketId :this.mysocketId, toSocketId : this.currentusers.chatList[i].socketId, openBox : this.oldchats.chatList[newindex].openBox, messages : []};
                        this.onlineList.push(inputa);
                    } 
                }else{
                    if(this.currentusers.chatList[i]._id != this.cid){
                        var inputb = {fromCustId: this.cid, toCustId : this.currentusers.chatList[i]._id, firstname: this.currentusers.chatList[i].firstname, lastname: this.currentusers.chatList[i].lastname, email: this.currentusers.chatList[i].email, fromSocketId :this.mysocketId, toSocketId : this.currentusers.chatList[i].socketId, openBox : 'close',  messages : []};
    ...
    open(){               
        ...
        console.log(document.getElementById('myModalc')); 
