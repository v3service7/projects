...
export class PanicService {
    ...
    public panicMyList(id) {
        const headers = new Headers();
        this.loadToken();
        headers.append('Authorization', this.authToken);
        headers.append('Content-Type', 'application/json');
        return this.http.get(globalVariable.url + 'panics/mypanic/' + id, {headers: headers})
        .map((response: Response) => {
            const user = response.json();
            return user;
        });
    ...
    public panicPendingList() {
        const headers = new Headers();
        this.loadToken();
        headers.append('Authorization', this.authToken);
        headers.append('Content-Type', 'application/json');
        return this.http.get(globalVariable.url + 'panics/pending', {headers: headers})
        .map((response: Response) => {
            const user = response.json();
            return user;
        });
